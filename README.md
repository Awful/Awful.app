...
[Awful][App Store] is an iOS 15.0+ app that's *Better Than Safari* for browsing the [Something Awful Forums][forums]. Its story is told in [its thread][current thread] (and the [thread before that][fourth thread] (and [the thread before that][third thread] (and [the thread before that][second thread] (and [the thread before that][first thread])))).
...
[current thread]: https://forums.somethingawful.com/showthread.php?threadid=4116244
[fourth thread]: https://forums.somethingawful.com/showthread.php?threadid=3837546
