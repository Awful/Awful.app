...
shot() { # shot <NN-slug> <url> <settle>
    local name="$1" url="$2" settle="${3:-4}"
    ...
    "$SCRIPT_DIR/nav.sh" "$UDID" "$url" "$settle"
...
shot 02-thread-list   "awful://forums/$FORUM_ID"                5
shot 03-posts-view    "awful://threads/$THREAD_ID/pages/last"   7
shot 04-bookmarks     "awful://bookmarks"                       5
shot 05-pm-list       "awful://messages"                        4
...
shot 19-poll-thread   "awful://threads/$POLL_THREAD_ID"     7
