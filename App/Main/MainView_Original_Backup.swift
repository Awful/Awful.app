...
/// caused by multiple @FoilDefaultStorage property wrappers
...
// Removed AwfulCustomNavigationWrapper - functionality consolidated into MainView
...
class MainCoordinatorImpl: MainCoordinator, ComposeTextViewControllerDelegate {
    ...
    func navigateToForum(_ forum: Forum) {
        ...
        if UIDevice.current.userInterfaceIdiom == .phone {
            ...
            path.append(forum)
            print("🔍 MainCoordinator: iPhone - appended to main path, count: \(path.count)")
        } else {
            // On iPad, use the sidebar navigation path (stacked navigation in sidebar)
            sidebarPath.append(forum)
            print("🔍 MainCoordinator: iPad - appended to sidebar path, count: \(sidebarPath.count)")
        }
    }
    
    func goBack() {
        if UIDevice.current.userInterfaceIdiom == .phone {
            // iPhone: pop from main path
            if !path.isEmpty {
                path.removeLast()
                print("🔙 MainCoordinator: iPhone - went back, path count: \(path.count)")
            }
        } else {
            // iPad: pop from sidebar path 
            if !sidebarPath.isEmpty {
                sidebarPath.removeLast()
                print("🔙 MainCoordinator: iPad - went back, sidebar path count: \(sidebarPath.count)")
            }
        }
    }
    
    func goForward() {
        // For now, simple implementation - could be enhanced with unpop stack later
        print("🔄 MainCoordinator: goForward called - not implemented yet")
    }
    ...
    func updateScrollPosition(scrollFraction: CGFloat) {
        ...
        if let lastIndex = navigationHistory.lastIndex(where: { $0 is ThreadDestination }),
           let threadDestination = navigationHistory[lastIndex] as? ThreadDestination {
            ...
            // IMPORTANT: Only update the NavigationPath for actual scroll position changes
            // For page changes (scrollFraction == 0.0), we only need to update navigation history
            // This prevents unnecessary view disappearing/appearing cycles
            
            // Check if we need to update the last item in the path for scroll position changes
            if path.count > 0 && scrollFraction != 0.0 {
...
struct MainView: View {
    ...
    private var iPadSidebar: some View {
        NavigationStack(path: $coordinator.sidebarPath) {
            ...
            .onChange(of: tabManager.selectedTab) { newTab in
                if newTab == .forums && tabManager.selectedTab == .forums {
                    coordinator.sidebarPath = NavigationPath()
                }
            }
        }
        .navigationDestination(for: Forum.self) { forum in
            Group {
                let _ = print("🔵 iPad Sidebar: NavigationDestination triggered for Forum: \(forum.name ?? "unnamed")")
        ...
        NavigationStack(path: $coordinator.path) {
            ...
            .navigationDestination(for: ThreadDestination.self) { destination in
                PostsViewWrapper(
                    ...
                    managedObjectContext: AppDelegate.instance?.managedObjectContext ?? NSManagedObjectContext.viewContext(),
                    coordinator: coordinator
                )
            }
        }
    ...
    // Removed duplicate iPadLayout - functionality is now in main body
    
    // Removed unused iPhoneLayout - functionality moved to main body
    
    // MARK: - iPad Layout
    // Removed iPadLayout computed property - iPad layout is now inline in main body

    // iPhone Layout is now inline in main body
    ...
    private func checkPrivateMessagePrivileges() {
        ...
        guard ForumsClient.shared.isLoggedIn && !settingsManager.canSendPrivateMessages else { return }
...
struct TabContentView: View, Equatable {
    ...
    static func == (lhs: TabContentView, rhs: TabContentView) -> Bool {
        // Only recreate if the essential properties change
        return lhs.tab == rhs.tab && 
               lhs.isEditing == rhs.isEditing
               // coordinator and environment changes will still trigger updates when needed
    }
...
// MARK: - Removed duplicate iPad Main View struct
// struct iPadMainView: View, Equatable { // REMOVED - was causing navigation conflicts
// } // End of removed struct

/*
// Commented out entire duplicate iPadMainView struct to fix navigation conflicts
struct iPadMainView: View, Equatable {
    ...
    static func == (lhs: iPadMainView, rhs: iPadMainView) -> Bool {
        // Only recreate if the essential properties actually change
        // Note: We skip selectedTab comparison since it's a Binding and changes frequently
        return lhs.isEditingBookmarks == rhs.isEditingBookmarks &&
               lhs.isEditingMessages == rhs.isEditingMessages &&
               lhs.isEditingForums == rhs.isEditingForums &&
               lhs.hasFavoriteForums == rhs.hasFavoriteForums &&
               lhs.canSendPrivateMessages == rhs.canSendPrivateMessages
               // Note: coordinator and theme changes will still trigger updates when needed
    }
    
    // Removed iPad navigation controller - using MainCoordinatorImpl only
    ...
    var body: some View {
        let _ = print("🎯 iPad iPadMainView: selectedTab = \(selectedTab.title), canSendPrivateMessages = \(canSendPrivateMessages)")
        let _ = print("🎯 iPad iPadMainView: Available tabs = \(MainTab.allCases(canSendPrivateMessages: canSendPrivateMessages).map { $0.title })")
        
        NavigationSplitView {
            // iPad Sidebar: NavigationStack wrapping TabView to handle forum navigation
            NavigationStack(path: $coordinator.sidebarPath) {
                TabView(selection: $selectedTab) {
                ForEach(MainTab.allCases(canSendPrivateMessages: canSendPrivateMessages)) { tab in
                    let _ = print("🎯 iPad TabView: Creating tab for \(tab.title)")
                    
                    // All tabs use regular content - NavigationStack is now at the TabView level
                    TabContentView(tab: tab, coordinator: coordinator, isEditing: getEditingState(for: tab))
                        .equatable()
                    .tabItem {
                        if let tabBarImage = tab.tabBarImage {
                        ...
                        Text(tab.title)
                            .font(.caption2) // Smaller text to save space
                    }
                    .tag(tab)
                }
            }
            ...
            .onChange(of: selectedTab) { newTab in
                ...
                if newTab == .forums && selectedTab == .forums {
                    // Clear sidebar navigation to go back to forums root
                    coordinator.sidebarPath = NavigationPath()
                }
            }
            }
            ...
            // iPad Detail: Only show posts and messages (WebKit render views)
            NavigationStack(path: $coordinator.path) {
                DetailView()
                    .environmentObject(coordinator)
                    .navigationDestination(for: ThreadDestination.self) { destination in
                    ...
                    .navigationDestination(for: PrivateMessageDestination.self) { destination in
                        Group {
                            let _ = print("🔵 iPad DetailView: navigationDestination triggered for PrivateMessage: \(destination.message.messageID)")
                            if let managedObjectContext = AppDelegate.instance?.managedObjectContext {
                                SwiftUIMessageView(
...
*/
// End of commented out duplicate iPadMainView struct
