...
struct SearchResultCard: View {
    ...
    var body: some View {
        ...
        .contextMenu {
            Button("Jump to Post") {
                AppDelegate.instance.open(route: .post(id: result.postID, .noseen))
            }
            
            Button("Copy Title") {
                UIPasteboard.general.string = result.threadTitle
            }
            
            Button("Copy URL") {
                if let url = URL(string: "https://forums.somethingawful.com/showthread.php?postid=\(result.postID)") {
