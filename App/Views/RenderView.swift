...
final class RenderView: UIView {
    ...
    private lazy var webView: WKWebView = {
        ...
        configuration.userContentController.addUserScript({
            let url = bundle.url(forResource: "loading-attribute-polyfill.min.js", withExtension: nil)!
            let script = try! String(contentsOf: url)
            return WKUserScript(source: script, injectionTime: .atDocumentEnd, forMainFrameOnly: false)
        }())
        ...
        let script = """
                        ...
                        function clearScrolLClasses(element){
                            element.classList.remove('initial');
                            element.classList.remove('outview-top');
                            element.classList.remove('outview-bottom');
                            element.classList.remove('inview-top');
                            element.classList.remove('inview-bottom');
                        }
                        ...
                        entries.forEach((entry, index) => {
                            ...
                            clearScrolLClasses(element);
                        
                            // Get root element (posts) coords
                            ...
                            // current post is not visible above the viewport
                            if (topBound < rootTop && bottomBound < rootTop) {
                                className = "outview-top";
                                element.style.backgroundColor = "blue";
                        
                            // current post is not visible below the viewport
                            } else if (topBound > rootBottom) {
                                className = "outview-bottom";
                                element.style.backgroundColor = "blue";
                            }
                        
                            // current post is visible, being scrolled up into the viewport
                            else if (topBound < rootBottom && bottomBound > rootBottom) {
                                className = "inview-bottom";
                                element.style.backgroundColor = "red";
                            }
                            
                            // current post is visible, being scrolled down into the viewport
