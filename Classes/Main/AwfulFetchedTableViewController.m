...
-(id) initWithEntity:(NSString*)entity predicate:(id)predicate sort:(id)sort sectionKey:(NSString*)sectionKeyPath {
    ...
    if (predicate) 
        if ([predicate isKindOfClass:[NSPredicate class]])
            ...
            [_request setPredicate:
             [NSPredicate predicateWithFormat:predicate]
             ];
    
    if (sort) {
        if ([sort isKindOfClass:[NSArray class]])
            [_request setSortDescriptors:sort];
        else if ([sort isKindOfClass:[NSSortDescriptor class]])
            [_request setSortDescriptors:[NSArray arrayWithObject:sort]];
    ...
    //imageQueue = dispatch_queue_create("images", DISPATCH_QUEUE_SERIAL);
...
- (NSFetchedResultsController *)fetchedResultsController {
    ...
    [[NSFetchedResultsController alloc] initWithFetchRequest:_request 
