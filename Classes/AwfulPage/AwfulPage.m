...
{       
    self.destinationType = AwfulPageDestinationTypeLast;
    [self loadPageNum:self.pages.totalPages];

}

- (void)viewDidUnload {
    [self setRefreshStopButton:nil];
    [super viewDidUnload];
}
...
-(void)swapToRefreshButton
{
    UIBarButtonItem *refresh = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemRefresh target:self action:@selector(hardRefresh)];
    refresh.style = UIBarButtonItemStyleBordered;
    NSMutableArray *ma = [NSMutableArray arrayWithArray:self.toolbarItems];
    int i = [ma indexOfObject:self.refreshStopButton];
    [ma replaceObjectAtIndex:i withObject:refresh];
    self.refreshStopButton = refresh;
    self.toolbarItems = ma;
}
...
-(void)swapToStopButton
{
    ...
    NSMutableArray *ma = [NSMutableArray arrayWithArray:self.toolbarItems];
    int i = [ma indexOfObject:self.refreshStopButton];
    [ma replaceObjectAtIndex:i withObject:stop];
    self.refreshStopButton = stop;
    self.toolbarItems = ma;
}
