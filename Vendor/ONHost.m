...
NSString * PunycodeEncode(NSString *aString);
NSString * PunycodeDecode(NSString *aString);
...
NSString * IDNEncodedHostname(NSString *aHostname)
{
    ...
    for (partIndex = 0; partIndex < partCount; partIndex++)
        [encodedParts addObject:PunycodeEncode([[parts objectAtIndex:partIndex] precomposedStringWithCompatibilityMapping])];
...
NSString * IDNDecodedHostname(NSString *anIDNHostname)
{
    ...
    for (labelIndex = 0; labelIndex < labelCount; labelIndex++) {
        ...
        decodedLabel = PunycodeDecode(label);
