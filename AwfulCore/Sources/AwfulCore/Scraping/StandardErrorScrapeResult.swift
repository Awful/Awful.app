...
public struct StandardErrorScrapeResult: ScrapeResult {
    ...
    public init(_ html: HTMLNode, url: URL?) throws {
        if
            let body = html.firstNode(matchingParsedSelector: .cached("body")),
            body.hasClass("standarderror")
        {
            let standard = body.firstNode(matchingParsedSelector: .cached("#content div.standard"))
            
            title = standard?
                .firstNode(matchingParsedSelector: .cached("h2"))?
                .textContent
                ?? ""
            ...
            message = standard
                .flatMap { (element: HTMLElement) -> HTMLElement? in element.firstNode(matchingParsedSelector: .cached("div.inner")) }
            ...
            let inner = html.firstNode(matchingParsedSelector: .cached(".standard .inner")),
